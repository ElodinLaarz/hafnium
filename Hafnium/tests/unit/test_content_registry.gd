...
func test_require_character_returns_wizard_definition():
	var wizard = ContentRegistry.require_character("class:wizard")
...
func test_legacy_class_lookup_uses_resource_registry():
	var wizard = ContentRegistry.require_legacy_character(ClassHandler.ClassName.WIZARD)
	assert_not_null(wizard, "Legacy enum lookup should resolve through registry")
	assert_eq(wizard.id, "class:wizard")


func test_require_enemy_returns_loot_enabled_definition():
	var slime = ContentRegistry.require_enemy("enemy:slime_basic")
