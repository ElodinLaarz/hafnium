...
func test_take_damage_returns_true_on_death() -> void:
	...
	s.max_health = 10
	s.current_health = 5
	var killed: bool = s.take_damage(5)
...
func test_resource_status_update_rate() -> void:
	var rs: Stats.ResourceStatus = Stats.ResourceStatus.new(Stats.ClassResource.BOMB, 3, 0.5)
...
func test_resource_status_max_clamp() -> void:
	var rs: Stats.ResourceStatus = Stats.ResourceStatus.new(Stats.ClassResource.BOMB, 1, 10.0)
