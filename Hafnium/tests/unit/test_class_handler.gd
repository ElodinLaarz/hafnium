...
func test_class_stats_wizard() -> void:
	var ch: Variant = CLASS_HANDLER_SCRIPT.new()
	var pc: Variant = ch.create_class(ClassHandler.ClassName.WIZARD)
...
func test_class_stats_barbarian() -> void:
	var ch: Variant = CLASS_HANDLER_SCRIPT.new()
	var pc: Variant = ch.create_class(ClassHandler.ClassName.BARBARIAN)
...
func test_get_attack_projectile_path() -> void:
	var ch: Variant = CLASS_HANDLER_SCRIPT.new()
	var wizard_path: Variant = ch.get_attack_projectile_path(ClassHandler.ClassName.WIZARD)
	assert_eq(
		wizard_path, "res://scenes/weapons/bullets/fireball.tscn", "Wizard should use fireball"
	)

	var none_path: Variant = ch.get_attack_projectile_path(ClassHandler.ClassName.NONE)
...
func test_wizard_attack_logic() -> void:
	...
	var second_success: Variant = pc.attack()
...
func test_use_resource_decrements_resource_status() -> void:
	var ch: Variant = CLASS_HANDLER_SCRIPT.new()
	var pc: Variant = ch.create_class(ClassHandler.ClassName.WIZARD)
	var bombs_before: Variant = pc.stats.resources["bomb"].current_resource
	var used: Variant = pc.use_resource("bomb", 1)
...
func test_use_resource_fails_when_insufficient() -> void:
	var ch: Variant = CLASS_HANDLER_SCRIPT.new()
	var pc: Variant = ch.create_class(ClassHandler.ClassName.WIZARD)
	var used: Variant = pc.use_resource("bomb", 99)
...
func test_barbarian_druid_attack_unimplemented() -> void:
	var ch: Variant = CLASS_HANDLER_SCRIPT.new()
	for cn: Variant in [ClassHandler.ClassName.BARBARIAN, ClassHandler.ClassName.DRUID]:
		var pc: Variant = ch.create_class(cn)
		assert_false(
			pc.attack(),
