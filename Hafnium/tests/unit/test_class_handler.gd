...
func test_get_attack_projectile_path():
	...
	assert_eq(wizard_path, "res://scenes/weapons/bullets/fireball.tscn", "Wizard should use fireball")
...
func test_use_resource_decrements_resource_status():
	...
	assert_eq(pc.stats.resources["bomb"].current_resource, bombs_before - 1, "Bomb count should decrease")
...
func test_barbarian_druid_attack_unimplemented():
	...
	for cn in [ClassHandler.ClassName.BARBARIAN, ClassHandler.ClassName.DRUID]:
		...
		assert_false(pc.attack(), "%s attack should return false until implemented" % ClassHandler.ClassName.keys()[cn])
