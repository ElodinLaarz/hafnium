...
- **Combat loop:** `CombatDirector`, `CharacterData` + `attack_projectile_id`, projectiles; right-click may be bomb **or** data-driven secondary spell (e.g. Wizard—on failure, bomb fallback); primary attacks are not mana-gated; shared `_roll_attack_crit`; crits + Luck additive bonus clamped by `GameConstants.CRIT_CHANCE_CAP`; Blood Mana wizard hook + `recompute_wizard_blood_mana(..., extra_base_mana_max)` for Magic progression; hit stop / screen shake / floating damage numbers.
- **Progression (run-scoped MVP slice):** `PlayerProgression` (XP thresholds, levels, five attributes); `EnemyData.experience_reward`; on kill, `RunContext` awards XP → queued level-up choices, tree pause/resume, `level_up_overlay.tscn`; `AttributeBonusService` applies Con/Dex/Magic/Luck/Willpower to stats; `LevelUpTooltipBuilder` + RichText stat tooltips.
...
| 2026-04-19 | Phase A (task 4): Blood Mana — Wizard max mana scales with missing HP (`CharacterData.blood_mana_bonus_pool`, `primary_spell_mana_cost`). `build_attack_logic` mana-gated primary when cost > 0; `PlayerClass.use_resource` emits `Stats.resource_changed`; `ClassHandler.recompute_wizard_blood_mana()` + health hook on Wizard. Mana progress bar (`ManaBarMargins` / `ManaBar` in `interface.tscn`) for Wizard; `update_interface.gd` + `draw_health.gd` (`PlayerHealth`) wire `RESOURCE_MANA`. `resources/characters/wizard.tres` initially `blood_mana_bonus_pool=8`, `primary_spell_mana_cost=1`. Tests: `tests/unit/test_blood_mana.gd`, `test_class_handler` wizard attack assertion. **Superseded 2026-04-20:** primary attacks no longer mana-gated; mana on secondary spell; see Wizard row below. |
| 2026-04-20 | **Experience & leveling (`feature/experience-leveling`):** `PlayerProgression` (levels, XP thresholds, five attributes); `PlayerCharacter.grant_experience` → `RunContext.enqueue_level_up_choice`; on enemy death all `active_players` gain `EnemyData.experience_reward` (`slime_basic`: 45). `GameConstants.LEVEL_UP_CHOICE_COUNT`; level-up CanvasLayer overlay (`level_up_overlay.tscn`) parented from `RunContext`, tree pause/resume; `resolve_level_up_choice` applies `PlayerProgression.increment_attribute` + `AttributeBonusService.apply`; `PlayerProgression.pick_random_attributes(count, rng)` optional `RandomNumberGenerator` for deterministic/tests. **Attributes:** Con (+max HP snapped to heart grid + Barb attack); Dex (+move speed + Druid attack); Magic (+max mana / wizard blood-mana integration + Wizard attack); Luck (+additive crit in `CombatDirector._roll_attack_crit`, clamped by `CRIT_CHANCE_CAP`); Willpower (attack cooldown clamp vs `AttributeBonusService` constants). `PlayerCharacter` baseline fields (`_baseline_*`) + `AttributeBonusService` after load; feel tuning refreshes movement baseline before reapply when baselines exist. |
| 2026-04-20 | **Level-up UI:** `LevelUpTooltipBuilder` + `StatChoiceButton._make_custom_tooltip` (RichText BBCode tooltips); Willpower copy tied to `AttributeBonusService` balance constants; class-colored bold lines for primary attack stats where relevant. |
| 2026-04-20 | **Hearts / HUD fixes:** `_on_health_changed` calls `check_and_create_hearts` so slots resize when max HP changes; `_snap_max_health_to_heart_grid` in `AttributeBonusService`; `dequeue_children` uses `remove_child` before `queue_free` (fixes hang when heart count changes). |
| 2026-04-20 | **Wizard primary vs secondary combat:** Removed mana gate from primary (`build_attack_logic`). `CharacterData`: `secondary_attack_projectile_id` / scene, `secondary_spell_mana_cost`, `secondary_damage_multiplier` (export docs match bomb fallback when spell cannot cast); `PlayerClass.secondary_attack_logic`, `secondary_attack()`, `has_secondary_spell()`, `get_secondary_attack_scene()`, `wizard_extra_base_mana_max` (Magic progression mana held across HP-driven `recompute_wizard_blood_mana`). Wizard `wizard.tres`: `primary_spell_mana_cost=0`; secondary fireball ID, 2 mana, 2.5× damage. Right-click: try secondary via `Common.cast_secondary_spell()` → `CombatDirector.fire_secondary_attack`, else bomb (`player_handler.gd`). **Projectile ordering:** instantiate + validate `Projectile`, then `secondary_attack()` mana/cooldown so failed casts do not burn resources. |
| 2026-04-20 | **Tests:** `test_blood_mana.gd` / `test_class_handler.gd` updated for primary-free and secondary mana; `test_run_context.gd` adds level-up queue / tree pause / seeded `pick_random_attributes`; full GUT suite preserved. |
