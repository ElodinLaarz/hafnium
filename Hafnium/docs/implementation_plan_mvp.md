...
| 3 | Battle Hardened (Barbarian passive) | S | After any hit, flag reduces next incoming damage by 1; reset on next hit. Hook via `RunContext` / health signals (`class_handler` or small `PassiveSystem`). |
