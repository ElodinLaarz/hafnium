...
- **Combat loop:** `CombatDirector`, `CharacterData` + `attack_projectile_id`, projectiles; right-click may be bomb **or** data-driven secondary spell (e.g. Wizard); crits + Luck modifier, hit stop / screen shake / floating damage numbers.
...
| 2026-04-20 | **Experience & leveling (`feature/experience-leveling`):** `PlayerProgression` (levels, XP thresholds, five attributes); `PlayerCharacter.grant_experience` → `RunContext.enqueue_level_up_choice`; on enemy death all `active_players` gain `EnemyData.experience_reward` (`slime_basic`: 45). Level-up CanvasLayer overlay on `RunContext`, tree pause/resume; `resolve_level_up_choice` applies `PlayerProgression.increment_attribute` + `AttributeBonusService.apply`. **Attributes:** Con (+max HP snapped to heart grid + Barb attack); Dex (+move speed + Druid attack); Magic (+max mana / wizard blood-mana integration + Wizard attack); Luck (+additive crit chance in `CombatDirector`); Willpower (attack cooldown clamp). `PlayerCharacter` baseline fields + `AttributeBonusService` after load / feel tuning. |
| 2026-04-20 | **Level-up UI:** `LevelUpTooltipBuilder` + `StatChoiceButton._make_custom_tooltip` (RichText BBCode tooltips); class-colored bold lines for primary attack stats where relevant. |
...
| 2026-04-20 | **Wizard primary vs secondary combat:** Removed mana gate from primary (`build_attack_logic`). `CharacterData`: `secondary_attack_projectile_id` / scene, `secondary_spell_mana_cost`, `secondary_damage_multiplier`; `PlayerClass.secondary_attack_logic`, `secondary_attack()`, `has_secondary_spell()`, `get_secondary_attack_scene()`. Wizard `wizard.tres`: primary `primary_spell_mana_cost=0`; secondary fireball ID, 2 mana, 2.5× damage; `recompute_wizard_blood_mana(pc, extra_base_mana_max)` for Magic progression. Right-click casts secondary when configured, else bomb (`player_handler.gd`). `Common.cast_secondary_spell()` → `CombatDirector.fire_secondary_attack`; **ordering fix:** resolve secondary `PackedScene`, instantiate and validate `Projectile`, then charge mana/cooldown (`secondary_attack()`), so failure after prep does not burn resources. |
| 2026-04-20 | **Tests:** `test_blood_mana.gd` / `test_class_handler.gd` updated for primary-free and secondary mana; full GUT suite preserved. |
