...
func get_room_half_extents(room_index: int) -> Vector2:
	var room_size_tiles: Vector2i = get_room_size_tiles(room_index)
...
func build_spawn_positions(spawn_count: int) -> Array[Vector2]:
	...
	var half_extents: Vector2 = get_room_half_extents(
...
func _spawn_wave(room_index: int) -> void:
	...
	var spawn_positions: Array[Vector2] = build_spawn_positions(wave_enemy_ids.size())
	var attempted_spawn_count: int = 0
	var successful_spawn_count: int = 0
	for i: int in range(wave_enemy_ids.size()):
		attempted_spawn_count += 1
		var enemy_id: String = wave_enemy_ids[i]
		var spawn_position: Vector2 = spawn_positions[i]
		var enemy: Enemy = Common.run_context.spawn_enemy(enemy_id, spawn_position)
		if enemy != null:
...
func _build_wave_enemy_ids(room_index: int) -> Array[String]:
	...
	for i: int in range(max(room_index, 1)):
...
func _clear_dynamic_children() -> void:
	if generated_room != null:
		for child: Node in generated_room.get_children():
			child.queue_free()

	if dynamic_entities != null:
		for child: Node in dynamic_entities.get_children():
			child.queue_free()
...
func _rebuild_room_geometry(room_index: int) -> void:
	if generated_room == null:
		return
	var half_extents: Vector2 = get_room_half_extents(room_index)
