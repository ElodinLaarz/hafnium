...
var graph_generator: Variant = ROOM_GRAPH_GENERATOR_SCRIPT.new()
...
func instantiate_room(room_id: String) -> Node2D:
	var room_data: Variant = ContentRegistry.require_room(room_id)
	if room_data == null or room_data.scene == null:
		return null

	var room_root: Variant = room_data.scene.instantiate()
...
func _apply_room_definition(room_root: Node, room_data: RoomData) -> void:
	...
	for node: Variant in room_nodes:
		if (
			node != null
